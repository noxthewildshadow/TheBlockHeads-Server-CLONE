...
echo "[7/8] Create economy data file and IP security file"
sudo -u "$ORIGINAL_USER" bash -c 'echo "{\"players\": {}, \"transactions\": []}" > economy_data.json' || true
sudo -u "$ORIGINAL_USER" bash -c 'echo "{\"admins\": {}, \"mods\": {}}" > ip_ranks.json' || true
chown "$ORIGINAL_USER:$ORIGINAL_USER" economy_data.json ip_ranks.json || true
